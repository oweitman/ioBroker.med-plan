...
vis.binds['med-plan'] = {
    ...
    /**
     * Generic entry point used by tplMed-planHelper (and can remain for backward compatibility).
     */
    ...
    /**
     * Dedicated entry point for Day Plan widget.
     */
    ...
    /**
     * Dedicated entry point for Multi Day Plan widget.
     */
    ...
    _createPlanWidgetCommon: function (widgetID, view, data, style, opts) {
        ...
        if (mode === 'multiday') {
            ...
            if (!isNaN(dp)) daysPast = dp;
            if (!isNaN(df)) daysFuture = df;
        }
        ...
        function rerender() {
            ...
            if (mode === 'day') {
                ...
                var inner = vis.binds['med-plan']._renderDayTable(
                    patientVM, patientObj, base, ymdKey, ymdLabel, showPatientName
                );
                ...
                for (var d = -daysPast; d <= daysFuture; d++) {
                    ...
                    var inner2 = vis.binds['med-plan']._renderDayTable(
                        patientVM, patientObj, day, ymdKey2, ymdLabel2,
        ...
        $div.off('click.medplan').on('click.medplan', 'button.mp-btn', async function () {
            ...
            try {
                ...
                if (!instance) throw new Error('Cannot derive instance from patientOid: ' + patientOid);
    ...
    _buildMedicationLookup: function (medListArr) {
        var map = {};
        if (!Array.isArray(medListArr)) return map;
            ...
            if (medsMap && typeof medsMap === 'object' && !Array.isArray(medsMap)) {
                Object.keys(medsMap).forEach(function (medicationId) {
                    ...
                    var normPkgs = pkgs.map(function (p) {
                        p = p || {};
                        var createdAt = (typeof p.createdTs === 'number')
    ...
    _renderDayTable: function (patientVM, patientObj, dt, ymdKey, ymdLabel, showPatientName) {
        ...
        if (showPatientName && patientVM.name) {
            ...
            html += '<div class="med-plan-title">' + vis.binds['med-plan']._escapeHtml(patientVM.name) + '</div>';
            html += '<div class="med-plan-subtitle">' + ymdLabel + '</div>';
            html += '</div>';
        } else {
        ...
        for (var i = 0; i < patientVM.medications.length; i++) {
            ...
            html += '<td class="med-plan-med-name">' + vis.binds['med-plan']._escapeHtml(med.medicationName) + '</td>';
            ...
            for (var j = 0; j < vis.binds['med-plan']._slots.length; j++) {
                ...
                var state = vis.binds['med-plan']._getIntakeStateFromPatient(patientObj, ymdKey, med.medicationId, slot.key);

                html += '<td class="med-plan-cell">';
                html += '<button type="button" class="mp-btn mp-state-' + state + '"' +
    ...
    _flashError: function ($root, msg) {
        ...
        setTimeout(function () {
            $e.fadeOut(200, function () { $e.remove(); });
        }, 2000);
    },
